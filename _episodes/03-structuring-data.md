...
sometimes lead to ambiguities). This is straightforward for a single level index, but becomes a little more tricky for hierarchical indexes. 

The simplest case is just to pass a single value or a tuple of values to select a specific row. For example `emissions.loc[3084.0, :]` would return emissions for all 
latitude and longitudes for the last month in 2007, or `emissions.loc[(3084.0, 89.5, 179.5), :]` would return the emissions at the North Pole for that month (zero hopefully!)
It's also possible to use slicing with a range of values, such as `emissions.loc[3072.0:3084.0, :]` or `emissions.loc[(3072.0, 89.5, 179.5):(3084.0, 89.5, 179.5), :]`.

> ## Challenge
> Try adding the following to you program and run it:
>
> ```python
> print(emissions.loc[(3072.0, 89.5, 179.5):(3084.0, 89.5, 179.5), :])
> ```
>
...
same meaning as the `range()`. One difference from `range()` is that `slice(None)` can be used to specify "all the contents."
...
> > ## Solution
> > ```python
> > print(emissions.loc[(3084.0, slice(40.5, 42.5), slice(-87.0, -73.5)), :])
> > ```
> > 
...
> >                            Total Emissions Per Month        FF
> > Month  Latitude Longitude                                     
> > 3084.0 40.5     -87.5                   2.705674e+11  0.000011
> >                 -86.5                   2.705674e+11  0.000011
...
> >        41.5     -87.5                   4.961416e+12  0.000200
> >                 -86.5                   2.664934e+11  0.000011
...
> >        42.5     -87.5                   0.000000e+00  0.000000
> >                 -86.5                   0.000000e+00  0.000000
